...
- **Decision: Modular Decoupling** — Separate UI drawing logic (`pkg/ui`) entirely from display manager messaging (`pkg/dbus`), binding them at the CLI entry point (`cmd/lightdm-greeter-ganapati`). This allows isolated unit testing of D-Bus protocols and independent UI mockup.
